...
{
    Matrix<double> phi(kNode, 7);
    double distance_square, cj, X;
    for (unsigned i = 0; i < kNode; i++)
    {
        for (unsigned j = 0; j < 7; j++)
        {
            ...
            for (unsigned k = 0; k < 4; k++)
            {
                ...
                {
                    X = v[j];
                    cj = kCj_v[0] + (kCj_v[1] - kCj_v[0]) * i / (kNode - 1);
                }
                else if (k == 1)
                {
                    X = a[j];
                    cj = kCj_a[0] + (kCj_a[1] - kCj_a[0]) * i / (kNode - 1);
                }
                else if (k == 2)
                {
                    X = q[j];
                    cj = kCj_q[0] + (kCj_q[1] - kCj_q[0]) * i / (kNode - 1);
                }
                else
                {
                    X = dq[j];
                    cj = kCj_dq[0] + (kCj_dq[1] - kCj_dq[0]) * i / (kNode - 1);
                }
...
{
    Matrix<double> dW_hat = -kEta * phi * s;
}
...
Matrix<double> controller(const Matrix<double> &J, const Matrix<double> &de, const Matrix<double> &s, const Matrix<double> &r, const Matrix<double> &phi, const Matrix<double> &W_hat)
{
    ...
    Matrix<double> control_input = phi.transpose() * W_hat - kK * s - J.transpose() * tau_bar;
}
